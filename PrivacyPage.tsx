    ...
    <div className="flex flex-col gap-8 pb-40 text-right" dir="rtl">
      ...
      <div className="mt-12 flex flex-col items-center select-none">
         <div 
           ...
           className={`w-full h-24 cursor-default active:cursor-wait transition-all duration-150 ${flash ? 'bg-red-900/20' : 'bg-transparent'}`}
           title="Developer Access (5 Clicks)"
         >
         </div>
         <div className={`w-12 h-1 rounded-full transition-colors duration-300 ${clickCount > 0 ? 'bg-red-600' : 'bg-white/5'}`}></div>
         <p className="text-[8px] text-gray-800 font-black uppercase tracking-[0.5em] mt-4 transition-opacity duration-300" style={{ opacity: clickCount > 0 ? 1 : 0.5 }}>
